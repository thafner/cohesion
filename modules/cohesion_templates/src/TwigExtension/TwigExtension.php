...
class TwigExtension extends \Twig_Extension {
  ...
  private function addComponentFrontEndBuilderMarkup($renderer, $_context, $componentInstanceUuid, $component_content_uuid = NULL, $component_content_id = NULL) {
    ...
    if($route_name == 'cohesion_elements.layout_canvas.build' && !isset($_context['hideContextualLinks']) && !isset($_context['isPreview']) && $this->hasDrupalPermission(["access contextual links", "access components"])) {

      if(!isset($_context['component_content'])) {
        $coh_start = [
          '#type' => 'container',
          '#attributes' => array(
            'data-coh-start' => array($componentInstanceUuid),
          ),
        ];
      ...
      if(!isset($_context['component_content'])) {
        $build[] = [
          '#type' => 'container',
          '#attributes' => array(
            'data-coh-end' => array($componentInstanceUuid),
          ),
        ];
      }
    }else {
  ...
  public function frontendBuilderDropzone($_context, $element_uuid, $stratStop) {
    $route_name = \Drupal::routeMatch()->getRouteName();
    if($route_name == 'cohesion_elements.layout_canvas.build' && !isset($_context['component_content']) && !isset($_context['hideContextualLinks']) && !isset($_context['isPreview']) && $this->hasDrupalPermission(["access contextual links", "access components"])) {
      return [
        '#type' => 'container',
        '#attributes' => array(
